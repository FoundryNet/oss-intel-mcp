"""Pay-per-query gate for oss-intel's paid tools — on-chain USDC micropayments on
Solana, fronted by a daily free tier. Same model as gov-contracts-mcp, but the
price VARIES per call (dependency_risk $0.02, project_health/trending_repos/
compare_packages $0.01, daily_brief $5), so the price is threaded through the gate
and bound into the payment memo.
...
(amount ≥ price, memo match, fresh, unused) → query runs. license_check is free.
...
logger = logging.getLogger("oss.pay")
